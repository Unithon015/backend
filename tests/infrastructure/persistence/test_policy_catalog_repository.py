...
class TestSqlAlchemyPolicyCatalogRepository(unittest.TestCase):
    def test_seeds_all_meta_policy_entries_idempotently(self) -> None:
        ...
        policy = session.query(PolicyCatalogEntryModel).filter_by(policy_code="META_HATE_SPEECH").one()
        self.assertTrue(policy.policy_summary)
        self.assertTrue(policy.detection_hints)
        self.assertIn("혐오", policy.match_keywords)
        self.assertEqual(policy.applicable_media_types, ["text", "image", "video"])
        session.close()
